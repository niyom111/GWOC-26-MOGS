...
const CaffeineComparison: React.FC = () => {
    return (
        <div className="w-full h-full min-h-[500px] flex items-center justify-center p-8">
            <div className="relative w-full max-w-md bg-[#0A0A0A] text-[#F9F8F4] p-10 shadow-2xl">
                {/* Header */}
                <div className="flex justify-between items-start mb-12">
                    <div>
                        <h4 className="text-sm font-sans uppercase tracking-[0.3em] text-[#CE2029] mb-2 font-bold">
                            Performance
                        </h4>
                ...
                <div className="flex gap-8 items-end h-[240px] mb-8 relative">
                    ...
                    <div className="w-1/2 flex flex-col gap-4 relative z-10 group">
                        <div className="text-right">
                            <span className="text-2xl font-mono text-zinc-500 group-hover:text-zinc-300 transition-colors">1.2%</span>
                        </div>
                        <div className="h-[100px] w-full bg-zinc-800 relative overflow-hidden">
                            <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20" />
                        </div>
                        <span className="text-xs uppercase tracking-widest text-zinc-500 font-medium">Standard Arabica</span>
                    </div>

                    {/* Bar 2: Rabuste */}
                    <div className="w-1/2 flex flex-col gap-4 relative z-10 group">
                        <div className="text-right">
                            <span className="text-4xl font-mono text-[#CE2029] font-bold">2.4%</span>
                        </div>
                        <div className="h-[200px] w-full bg-[#CE2029] relative overflow-hidden shadow-[0_0_30px_rgba(206,32,41,0.3)]">
                            <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20" />
                            <motion.div
                                initial={{ top: "100%" }}
                                whileInView={{ top: "-100%" }}
                                transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
                                className="absolute left-0 right-0 h-1/2 bg-gradient-to-b from-transparent via-white/20 to-transparent"
                            />
                        </div>
                        <span className="text-xs uppercase tracking-widest text-[#CE2029] font-bold">Rabuste Pure</span>
                    </div>
                </div>
                ...
                <div className="pt-8 border-t border-white/10">
                    <p className="font-sans text-sm text-zinc-400 leading-relaxed">
                        <strong className="text-white">Translation:</strong> You drink half as much to get twice as much done. Efficiency is the ultimate luxury.
                    </p>
                </div>
            </div>
        </div>
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10 w-full grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                ...
                <div className="flex flex-col gap-16">
                        ...
                        {words.map((word, i) => {
                            ...
                            return (
                                <motion.span
                                    key={i}
                                    style={{ opacity, y }}
                                    className={`text-5xl md:text-7xl lg:text-8xl font-black tracking-tight leading-[0.9] ${word.includes('.') ? 'text-[#CE2029]' : 'text-[#1A1A1A]'}`}
                                >
                                    {word}
                                </motion.span>
                            );
                    ...
                    <motion.div
                        ...
                        className="pl-8 border-l-4 border-[#CE2029]"
                    >
                        ...
                        <p className="text-lg md:text-xl font-medium font-sans text-zinc-800 leading-relaxed mb-6">
                            While the world obsesses over delicate floral notes, we obsess over <span className="underline decoration-[#CE2029]/30 decoration-2 underline-offset-4">raw horsepower</span>.
                        </p>
